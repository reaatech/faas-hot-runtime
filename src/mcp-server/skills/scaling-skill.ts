...
export class ScalingSkillHandler {
  ...
  private validateUpdateScalingPolicyArgs(
    args: Record<string, unknown>,
  ): UpdateScalingPolicyParams {
    if (typeof args['function'] !== 'string') {
      throw new McpError(ErrorCode.InvalidParams, 'function is required and must be a string');
    }
    const result: UpdateScalingPolicyParams = { function: args['function'] };
    if (args['min_pods'] !== undefined) {
      if (typeof args['min_pods'] !== 'number')
        throw new McpError(ErrorCode.InvalidParams, 'min_pods must be a number');
      result.min_pods = args['min_pods'];
    }
    if (args['max_pods'] !== undefined) {
      if (typeof args['max_pods'] !== 'number')
        throw new McpError(ErrorCode.InvalidParams, 'max_pods must be a number');
      result.max_pods = args['max_pods'];
    }
    if (args['target_utilization'] !== undefined) {
      if (typeof args['target_utilization'] !== 'number')
        throw new McpError(ErrorCode.InvalidParams, 'target_utilization must be a number');
      result.target_utilization = args['target_utilization'];
    }
    if (args['scale_up_threshold'] !== undefined) {
      if (typeof args['scale_up_threshold'] !== 'number')
        throw new McpError(ErrorCode.InvalidParams, 'scale_up_threshold must be a number');
      result.scale_up_threshold = args['scale_up_threshold'];
    }
    if (args['scale_down_threshold'] !== undefined) {
      if (typeof args['scale_down_threshold'] !== 'number')
        throw new McpError(ErrorCode.InvalidParams, 'scale_down_threshold must be a number');
    ...
    if (args['cooldown_seconds'] !== undefined) {
      if (typeof args['cooldown_seconds'] !== 'number')
        throw new McpError(ErrorCode.InvalidParams, 'cooldown_seconds must be a number');
  ...
  private async getScalingPolicy(
  ...
  private async updateScalingPolicy(
    params: UpdateScalingPolicyParams,
  ): Promise<{ content: Array<{ type: string; text: string }> }> {
    ...
    const result = {
      ...
      policy: {
        ...
        target_utilization:
          ...
          this.scalingController.getTargetUtilization(params.function) ??
          0.7,
        scale_up_threshold:
          params.scale_up_threshold ?? this.scalingController.getScaleUpThreshold() ?? 0.8,
        scale_down_threshold:
          params.scale_down_threshold ?? this.scalingController.getScaleDownThreshold() ?? 0.3,
        cooldown_seconds:
          params.cooldown_seconds ?? this.scalingController.getCooldownSeconds() ?? 60,
      },
    };
  ...
  private async getScalingHistory(
    params: GetScalingHistoryParams,
  ): Promise<{ content: Array<{ type: string; text: string }> }> {
    const events: Array<{
      timestamp: string;
      function: string;
